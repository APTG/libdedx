...
static void gold_section(dedx_internal_bethe_model bet, dedx_internal_bethe_gold *gold, int *err);
...
float dedx_internal_calculate_bethe_energy(
    ...
    dedx_internal_bethe_model *bet = ws->bet;
    int err = 0;
    if (gold != NULL && bet != NULL
        ...
        if (ws->bet != NULL)
            free(ws->bet);
        ws->gold = calloc(1, sizeof(dedx_internal_bethe_gold));
        ws->bet = calloc(1, sizeof(dedx_internal_bethe_model));
        gold = ws->gold;
        bet = ws->bet;
    }
    // float mass = 940*PA;
...
static float evaluate_bethe_model_LEext(float PT, dedx_internal_bethe_model bet, dedx_internal_bethe_gold gold,
    ...
    dedx = 0.307075 * bet.TZ0 / bet.TA0 * (ZPART2 / beta_2)
...
static float evaluate_bethe_model(float PT, dedx_internal_bethe_model bet, int *err) {
    ...
    float DEDXM = 0.307075 * bet.TZ0 / bet.TA0 * (ZPART2 / beta_2)
                  * (log((1.022E+06 / bet.potentiale) * gamma_beta) - beta_2 - delta / 2.0);
