 ...
 *   - DEDX_ICRU falls back to the Bethe-Bloch formula for elements/compounds no
 *     tabulated report covers, while report-specific programs (PSTAR, ASTAR, ICRU49,
 *     ICRU73, ...) still fail those combinations as they should. */
...
/* ...while DEDX_ICRU falls back to the Bethe-Bloch formula for the same element,
 * both directly and via Bragg-additivity decomposition of a Boron compound. */
static int test_icru_bethe_fallback(void) {
    int failures = 0;
    int err;
    dedx_workspace *ws;
    dedx_config *cfg;
    ...
    err = 0;
    dedx_load_config(ws, cfg, &err);
    failures += check_err(err, DEDX_OK, "ICRU+proton+Boron load");
    if (err == DEDX_OK) {
        stp = dedx_get_stp(ws, cfg, 100.0f, &err);
        if (err != DEDX_OK || stp <= 0.0f) {
            fprintf(stderr, "FAIL ICRU+proton+Boron stp: err=%d stp=%f\n", err, (double) stp);
            failures++;
        }
    }
    dedx_free_config(cfg, &err);
    ...
    err = 0;
    dedx_load_config(ws, cfg, &err);
    failures += check_err(err, DEDX_OK, "ICRU+proton+BoronCarbide load");
    failures += check_err(cfg->bragg_used, 1, "ICRU+proton+BoronCarbide should decompose via Bragg additivity");
    dedx_free_config(cfg, &err);
    dedx_free_workspace(ws, &err);

    /* PSTAR still can't serve the same compound: Boron is not in its table and
     * PSTAR does not get the Bethe fallback. */
    ws = dedx_allocate_workspace(1, &err);
    cfg = calloc(1, sizeof(dedx_config));
    cfg->program = DEDX_PSTAR;
    cfg->ion = DEDX_PROTON;
...
static int test_material_list_for_ion(void) {
    ...
    materials[len] = -1;
    failures += check_present(materials, DEDX_BORON, "ICRU+proton for_ion");
    failures += check_present(materials, DEDX_BORON_CARBIDE, "ICRU+proton for_ion");
...
static int test_fill_material_list_for_ion(void) {
    ...
    int wrap_err = dedx_fill_material_list_for_ion(DEDX_ICRU, DEDX_PROTON, wrapped);
    dedx_get_material_list_for_ion(DEDX_ICRU, DEDX_PROTON, direct, DEDX_MAX_MATERIAL_LIST, &direct_len, &err);
...
int main(void) {
    ...
    failures += test_icru_bethe_fallback();
