...
int test_run(int test, dedx_config *config, char *text, float input_value, float result, float err_accept);
void spacer(void);
void skip(int *nr);
...
int test_run(int test, dedx_config *config, char *text, float input_value, float result, float err_accept) {
    ...
    if ((test_result > result * (1 + err_accept)) || (test_result < result * (1 - err_accept))) {
        printf("%-70s : %.5e - %.5e ... OUT OF BOUNDS %+.1f\n",
               text,
               test_result,
               result,
               (test_result - result) * 1e3 / result);
        err_count++;
    } else
