...
typedef struct {
    PyObject_HEAD dedx_config *config;
    dedx_workspace *ws;
    int err;

} Libdedx;

static void Libdedx_dealloc(Libdedx *self) {
    int err = 0;
...
static PyObject *Libdedx_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
...
static int Libdedx_init(Libdedx *self, PyObject *args, PyObject *kwds) {
    return 0;
}
static PyObject *Libdedx_get_stp(Libdedx *self, PyObject *args) {
    float energy, stp;
...
static PyMethodDef Libdedx_methods[] = {
    {"get_stp", (PyCFunction) Libdedx_get_stp, METH_VARARGS, "Return stp"},
    {"get_error", (PyCFunction) Libdedx_get_error, METH_NOARGS, "Return error string"},
    {NULL} /* Sentinel */
};
...
#endif
PyMODINIT_FUNC initlibdedx(void) {
    PyObject *m;

    if (PyType_Ready(&LibdedxType) < 0)
        return;

    m = Py_InitModule3("libdedx", module_methods, "Example module that creates an extension type.");
