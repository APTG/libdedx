...
int _dedx_load_bethe2(
    stopping_data *data, float PZ, float PA, float TZ, float TA, float rho, float pot, float *energy, int *err);
...
dedx_workspace *dedx_allocate_workspace(unsigned int count, int *err) {
    ...
    *err = DEDX_OK;
    ...
    temp->loaded_data = calloc(count, sizeof(_dedx_lookup_data *));
