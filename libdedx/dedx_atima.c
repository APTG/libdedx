...
float _dedx_calculate_atima_energy(float energy, dedx_config *config) {
    ...
    if (energy <= 10) {
        ...
        dedx = (1 - factor) * _dedx_sezi(e_tot, PZ, PA, TZ, TA) + factor * _dedx_bethek(e_tot, PZ, PA, TZ, TA, pot, rho)
               + _dedx_dedxela(e_tot, PZ, PA, TZ, TA);
    } else {
...
float _dedx_bethek(float energy, int PZ, int PA, int TZ, float TA, float pot, float rho) {
    ...
    if (eta >= 0.13) {
        c = (.422377 * pow(eta, -2) + .0304043 * pow(eta, -4) - .00038106 * pow(eta, -6)) * 1e-6 * pow(pot, 2)
            + (3.858019 * pow(eta, -2) - 0.1667989 * (pow(eta, -4)) + 0.00157955 * (pow(eta, -6))) * 1.0e-9
                  * pow(pot, 3);
        f2 = f2 - c / TZ;
    }
...
float _dedx_sezi(float energy, int PZ, int PA, int TZ, float TA) {
    ...
    if (PZ == 1) {
        ...
        if (e < he0)
            E = he0;
        B = log(E);
        A = 0.2865 + 0.1266 * B - .001429 * B * B + 0.02402 * B * B * B - 0.01135 * B * B * B * B
            + 0.001475 * B * B * B * B * B;
        if (A < 30)
            heh = 1 - exp(-1 * A);
        else
