...
int main(int argc, char *argv[]) {
    ...
    if (prog_select == 3) {
        sprintf(progname, "BETHE_EXT00 using mean exitation potential I = %.2feV", ivalue);
        for (i = 0; i < TRIP_IONS; i++) {
    ...
    if (prog_select == 3)
        sprintf(buffer, "DEDX_BETHE_EXT00 using mean excitation potential I = %.2f eV", cfg[0]->i_value);
    ...
    for (j = 0; j < TRIP_IONS; j++) // the number of projectiles Check the number! Boundary value analysis CORRECT!!
    {
        ...
        for (i = 0; i < TRIP_ETABLE; i++) // the number of energy entries
        {
            ...
            if (Z[j] == 2) {
                if (trip_energy_table[i] > 250) {
                    range = masses[j] * get_range(ws, cfg[j], &err, a, 250, acc, eps)
                            + masses[j] * get_range(ws, cfg[TRIP_IONS], &err, 250, trip_energy_table[i], acc, eps);
                } else {
