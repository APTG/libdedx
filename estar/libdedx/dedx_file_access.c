...
int attemps = 0;
float _dedx_read_mat_data(int target,int items, char state, int *err);

void _dedx_get_estar_data(int target, int * nc, float * bd, unsigned int * items)
{
	*items = 0;
	char file[] = "estar_data";
	char path[80];
	char str[100];
	char **temp;
	int i = 0;
	*items = 0;
	...
	while(!feof(fp))
	{
		...
		if(str[0] != '#')
			continue;
		temp = _dedx_split(&str[1],':',&items, 100);
...
float _dedx_read_mat_data(int target, int item, char state, int *err)
{
	float value = 0.0;
	...
	while(!feof(fp))
	{
		...
		if(atoi(temp[0]) == target)
		{
			if(items == 5)
			{
				if(state == atoi(temp[4]))
				{
					value = atof(temp[item]);
				}
			}
			else
				value = atof(temp[item]);
...
{
	return _dedx_read_mat_data(id,1,DEDX_DEFAULT,err);
}
...
{
	return _dedx_read_mat_data(target,3,DEDX_DEFAULT,err);
}
float _dedx_get_i_value(int target,int state, int * err)
{
	return _dedx_read_mat_data(target,2,state,err);
}
